...
export class DerivAPIClient {
   ...
   * Made optional - no longer required for basic functionality
   */
  async authorize(token: string): Promise<any> {
    if (!token) {
      console.log("[v0] Authorization skipped - no token provided")
      this._isAuthorized = true // Mark as authorized anyway
