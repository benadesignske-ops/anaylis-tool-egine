...
export function AutomatedTradesTab({ theme = "dark" }: AutomatedTradesTabProps) {
  const { ws, isLoggedIn, balance: authBalance, accountType } = useDeriv()
  ...
  useEffect(() => {
    ...
    const initConnection = async () => {
      try {
        ...
        if (!ws.isConnected()) {
          ...
          let attempts = 0
          while (!ws.isConnected() && attempts < 20) {
        ...
        subscriptionId = await ws.subscribeTicks(selectedMarket, (tickData) => {
    ...
    return () => {
      if (subscriptionId) {
        ws.unsubscribe(subscriptionId).catch(() => {})
      }
      isInitializingRef.current = false
    }
