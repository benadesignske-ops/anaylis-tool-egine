...
export function AutoRunBot({
  ...
  const { ws } = useDeriv()
  const apiClientRef = useRef<any | null>(null)
  ...
  const runTradingLoop = useCallback(async () => {
    ...
    if (result.isWin) {
      ...
      currentStakeRef.current = Math.min(currentStakeRef.current * config.martingale, 1000) // Placeholder for balance logic
    }
  ...
  }, [executeSingleTrade, config, logJournal])

  const startBot = useCallback(() => {
    if (!ws) {
      logJournal("Please connect to Deriv first", "error")
      return
    }
    ...
    runTradingLoop()
  }, [ws, config.stake, config.symbol, logJournal, runTradingLoop])
