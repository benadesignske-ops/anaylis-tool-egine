...
export function useDeriv(initialSymbol = "R_100", initialMaxTicks = 100) {
  ...
  return {
    ...
    currentDigit,
    tickCount,
    // Return as 'analysis' but also provide 'analysisData' alias for compatibility
    analysis,
    analysisData: analysis || {
      digitFrequencies: [],
      patterns: { hot: [], cold: [], consecutive: [] },
      statistics: { mean: 0, median: 0, mode: 0, stdDev: 0, range: { min: 0, max: 0 } },
      signals: [],
      proSignals: [],
    },
    signals: signals || [],
    ...
    aiPrediction,
    // Return as 'symbol' but also provide 'selectedSymbol' alias
    symbol,
    selectedSymbol: symbol,
    setSelectedSymbol: changeSymbol,
    maxTicks,
    // Return as 'availableSymbols' but also provide 'symbols' alias
    availableSymbols,
